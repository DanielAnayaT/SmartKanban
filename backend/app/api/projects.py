...
def create_project(project: ProjectCreate, owner_id: int, current_user= Depends(get_current_user), project_service: ProjectService = Depends(get_project_service)):
    try:
        return project_service.create_project(project.name, project.description, owner_id, current_user.id)
    except ValueError as e:
        raise HTTPException(status_code=500, detail=str(e))

# Listar proyectos de un usuario
@router.get("/", response_model=list[ProjectOut])
def list_projects(owner_id: int, current_user= Depends(get_current_user), project_service: ProjectService = Depends(get_project_service)):
    try:
        return project_service.list_projects_by_owner(owner_id, current_user.id)
